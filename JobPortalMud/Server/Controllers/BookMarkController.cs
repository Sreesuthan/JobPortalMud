...
namespace JobPortalMud.Server.Controllers
{
    ...
    public class BookMarkController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<BookMark>>> GetAllBookmarkedjob(string user)
        {
            ...
            IEnumerable<BookMark> bookmarkedJobs = await connection.QueryAsync<BookMark>("Select bj.Id, j.CompanyName, bj.JobId, j.Title, bj.UserName, j.JobType, j.CompanyName, j.Address, j.country, State from BookmarkedJobs as bj inner join AspNetUsers as u on bj.UserName = u.UserName inner join jobs as j on bj.JobId = j.JobId where u.UserName=@UserName", new { UserName = user });
